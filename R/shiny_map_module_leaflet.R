...
mini_mapServer <- function(id, polygonLayer = reactive(NULL), polygonNames = reactive((NULL))) {
  moduleServer(id, function(input, output, session) {
    ...
    output$subStratTable<-DT::renderDataTable({
      ...
      DTtab<-DT::datatable(tabl, smTab, selection = "none", rownames = T, escape = T, width = 30,
...
# # Main app UI
# ui <- fluidPage(
#   #mini_ampUI("mapModule"),
#   actionButton("mapModal", "Map")
# )
#
# # Main app server function
# server <- function(input, output, session) {
#
#   observeEvent(input$mapModal, {
#
#     shiny::showModal(
#       shiny::modalDialog(
#         title = "Select",
...
#   })
#
#   polySelFullOuter<-mini_mapServer("mapModule", polygonLayer = reactive(Boundaries), polygonNames = reactive("NAME_1"))
#
#   observe({
#     print(polySelFullOuter$ID)
#   })
# }
#
# shinyApp(ui, server)
