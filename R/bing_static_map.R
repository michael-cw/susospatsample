...
getStaticMapAsRaster<-function(shape = shape,
  ...
  if(byShape){
    ...
    if(singleMap) {
      ...
      ext<-keepShape %>% st_transform(4326) %>% st_bbox()
      singleMap<-basemaps::basemap_stars(ext = ext, map_service = mapservice, force = F,
    ...
    file_name<-shape %>% select(.data[[name_var]]) %>% st_set_geometry(NULL)
    file_name<-file_name[1,1]
