...
writeSFtoDB<-function(object = NULL, dbname = NULL, host = "localhost",
  ...
  if(DBtype == "pg"){
    ...
    if(!listTables){
      ...
      st_write(object, dsn = con, layer = fn,
    ...
    if(!listTables){
      if(dir.exists(localpath)) {
        st_write(object, dsn = localpath, layer = paste0(fn, ".shp"),
...
readSHPfromDB<-function(dbname = NULL, host = "localhost",
  ...
  if(DBtype == "pg"){
    ...
    if(!listTables){
      ...
      allTables<-st_read(dsn = con, layer = fn,
                         quiet = T)
      ## 4. Disconnect
      RPostgres::dbDisconnect(con)
    ...
    if(!listTables){
      if(dir.exists(localpath)) {
        allTables<-st_read(dsn = file.path(localpath, fn),
                           quiet = T)
      } else {
