...
main_server <- function(input, output, session) {
  ...
  #################################################################################
  ##              Data Base Set-up
  storemode <- reactiveVal(NULL)
  shppath <- reactiveVal(NULL)
  raspath <- reactiveVal(NULL)
  DBname <- reactiveVal(NULL)
  DBhost <- reactiveVal(NULL)
  DBuser <- reactiveVal(NULL)
  DBpass <- reactiveVal(NULL)
  ...
  shp.info.tabl <- reactive({
    ...
    shiny::validate(
      need(shp.info, message = "Please load shape file first!"),
  ...
  observe({
    ...
    if (nrow(pts) > getOption("pointsLimit")) {
      shiny::showNotification(
        paste0(
          "The number of points exceeds the limit for map display, therefore
                                     the maps will be aggregate to the provided regions. Aggregating Map for ",
          nrow(pts),
          " units. This may take a while"
        ),
        closeButton = F,
  ...
  observe({
    ...
    if (getOption("mapwidget.option") == "mapdeck") {
      mapModuleSvr(
        ...
        updateGroup = reactive({
          (ifelse(input$strat == "Yes", "stratum_numeric", ""))
        }),
        polyId = reactive(NULL), z_var = reactive(NULL)
      )
    } else if (getOption("mapwidget.option") == "leaflet") {
  ...
  observe({
    ...
    if (getOption("mapwidget.option") == "mapdeck") {
      mapModuleSvr(
        ...
        polyId = reactive(NULL), z_var = reactive({
          POP
        })
      )
    } else if (getOption("mapwidget.option") == "leaflet") {
  ...
  observeEvent(rastaCreateReact(),
               {
                 ...
                 withProgress(
                   message = paste(
                     "Raster creation started with variable",
  ...
  observe({
    ...
    withProgress(message = "Loading raster, this may take a while!", value = 0, {
      ...
      if (input$popUpType == 1) {
        ...
        shiny::validate(
          need(pop_raster, message = "Select file first!"),
        ...
        shiny::validate(
          need(input$pop_file_raster, message = "Select file first!"),
          need(harare_landuse3_type1, message = "Select file first!")
        )
      ...
      if ((input$aggregateRaster) > 1) {
        ...
        pop_raster <- raster::aggregate(pop_raster,
                                        fact = (input$aggregateRaster),
                                        fun = sum,
                                        expand = TRUE,
                                        na.rm = TRUE
        )
  ...
  output$popInfoTab <- DT::renderDataTable({
    ...
    DT::datatable(as.data.frame(tabl), smTab,
  ...
  observeEvent(input$modify,
               {
                 ...
                 modalHeadFull <- paste0(
                   "<center><font color='#7f0000'><big>Attention, this will create the following ",
                   modalHead,
                   ":</big></font></center>"
                 )
                 showModal(modalDialog(
                   ...
                   conditionalPanel(
                     ...
                     fluidRow(
                       column(
                         4,
                         radioButtons("subSTRAT", "Subset Regions",
  ...
  harare_landuse3_type1_sub <- reactive({
    ...
    if (nrow(DF) != length(unique(DF[, names(DF) == input$`strVarSel-strat_var`, drop = T]))) {
      DF <- DF %>%
        group_by(.data[[input$`strVarSel-strat_var`]]) %>%
        summarise(Count = n())
                   ...
                   if (isolate(input$sampType == "Random Grid")) {
                     ...
                     if (input$strat == "No") {
                       ...
                       withProgress(message = "Creating Grid, this may take a while!", value = 0, {
                         ...
                         sp_grd_strat$GRIDID <- sprintf(
                           "Lat%dLon%d", floor(sp_grd_strat$Y / input$cell_side),
                       ...
                       popSizeStrat <- sp_grd_strat %>%
                         dplyr::group_by(stratum_numeric) %>%
                         dplyr::summarise(Pop = n(), Cells = n())
                       new_pop$popSizeStrat <- popSizeStrat
                     }
                     ########################################################################
                   ...
                   withProgress(message = "Transforming Data, this may take a while!", value = 0, {
                     if (isolate(input$sampType == "Random Grid")) {
                       ...
                       if (input$strat == "No") {
                         ...
                         if (is.data.table(final)) {
                           ...
                           ras_points <- rbindlist(final)
                         }
                         popSize <- ceiling(sum(ras_points$V1))
                         popSizeStrat <- ras_points %>%
                       ...
                       if (isolate(input$popUpType == 1)) {
                         ...
                         if (isolate(input$strat == "No")) {
                           ...
                           unlink(tmpFile)
                           writeRaster(pop_raster, tmpFile)
                           rm(pop_raster)
                           ## ii. read Proxy
                     ...
                     if (isolate(input$popUpType == 1 & isolate(input$sampType == "Random Cluster"))) {
                       ...
                       if ("stars" %in% class(pop_raster)) {
                         ...
                         if (isLonLat(pop_raster)) {
                           resRas <- distHaversine(
                             c(
                               extent(pop_raster)[1],
                               extent(pop_raster)[3]
                             ),
                             c(
                               (extent(pop_raster)[1] + res(pop_raster)[1]),
                               extent(pop_raster)[3]
                             )
                           )
        ...
        fluidPage(
          ...
          fluidRow(
            ...
            column(
              4,
              conditionalPanel(
                "input.popCreate=='Yes'|input.sampType=='Random Cluster'",
                radioButtons("sampPPS", "Apply PPS sampling?",
                             choices = c("Yes", "No"), inline = T,
                             selected = "No"
                ),
                conditionalPanel(
                  "input.sampPPS=='Yes' & input.sampType=='Random Grid'",
                  numericInput("minCell", "Minimum Grid Value?", 0, step = 1, width = "100%")
                ),
                conditionalPanel(
                  "input.catRaster=='Yes'& input.sampPPS=='Yes'",
                  selectizeInput("catRaster_domCat", "Select Dominant Category",
                                 choices = rasCat,
                                 options = list(
                ...
                conditionalPanel(
                  "input.sampType=='Random Cluster'& input.sampPPS=='Yes' & input.popCreate=='No'",
                  selectizeInput("rcluster_MOS", "Select Size Variable",
                                 choices = varNam,
                                 options = list(
                                   placeholder = "Select variable bellow",
                                   onInitialize = I('function() { this.setValue(""); }')
            ...
            column(
              ...
              conditionalPanel(
                # "input.popCreate=='Yes'|input.sampType=='Random Cluster'",
                "true",
                radioButtons("timeDistr", "Distribute across time period?",
                             choices = c("Yes", "No"), inline = T,
                             selected = "No"
                ),
                conditionalPanel(
                  "input.timeDistr=='Yes'",
                  numericInput("nPeriods", "Number of time periods?",
                               value = 0, step = 1, min = 0, max = 12)
                )
              )
            )
          ),
  ...
  observeEvent(input$conf3, {
    ...
    if (input$strat == "Yes" & input$sampSizeStrat == "Yes") {
      shiny::validate(
        need(tabl, message = "Nothing selected"),
  ...
  observeEvent(input$conf3, {
    ...
    if (input$sampPPS == "No" & input$strat == "No") {
      ...
      shiny::validate(
        need(is.numeric(sampSize), message = T),
        need(
          (length(sampSize) == length(unique(pop_raster_shp$stratum_numeric))) |
            length(sampSize) == 1,
          message = F
        )
      )
  ...
  ##                                        T I L E S                                     ##
  ##########################################################################################
  TPKpath <- modal_createbasemap_server("mapfordwl",
  ...
  zipfilepath <- modal_createshape_server("shapesfordwl",
                                          sample_seed = sample_seed,
                                          shape_boundaries = shapefileinput,
                                          sampType = reactive({
                                            input$sampType
  ...
  sampleDataForDownload <- reactive({
    withProgress(
      message = "Generating Sample for Download",
      value = 20,
      {
        ##################
        ...
        ## Bind to data
        samp_raster_shp <- cbind(
          st_sf(geometry = samp_raster_shp),
        ...
        if(input$timeDistr=="Yes" && input$nPeriods>0) {
          ## Check if there is at lease 1 unit for each time period
          nperiods<-input$nPeriods
          ## No distribution without sufficient sample
          if(nrow(samp_data)<nperiods) {
            showNotification("Not enough units per time period, continouing without
                         time distribution!", type = "error")
          } else {
            incProgress(20)
            ## Generate temporary ID for sample
            samp_data[,tmpID:=1:.N]
            samp_data<-split_and_write_spatbalance_dt(
              samp_data, 
              nperiods, 
              "Period", 
              "gps_LAT" ,
              ...
              NULL, 
              T)
            samp_data<-data.table::rbindlist(samp_data, idcol = "TimePeriod")
            ## Remove temporary ID
            samp_data[,tmpID:=NULL]
  ...
  report_content <- reactive({
    ...
    withProgress(
      ...
      value = 0,
      {
        ...
        seed <- sample_seed()
        basic_char <- as.data.frame(cbind(
          c("Sampling Type", "Stratification", "Population", "Population Type", "Seed"),
        ...
        full_content$sec_para <- list(
          sec1 = list(para1 = paste0(
            "The following Boundaries have been used for the current sample. Within these boundaris,",
            toupper(input$sampType), " sampling has been applied."
          )),
          sec2 = list(
            para1 = "The following population information has been provided for the current sample.",
            para2 = "The following subset of regions was used for the sample."
          ),
          sec3 = list(
            para1 = "The final design parameters are as follows:",
  ...
  #########################
  # SECOND STAGE SAMPLE
  gridPointsSpatial2stage <- modal_spsample2stage_server("my_modal", shape_boundaries = reactive({
    NULL
  }))
